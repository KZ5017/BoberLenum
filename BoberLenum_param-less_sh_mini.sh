...
list_suid() { find / -perm -04000 -ls 2>/dev/null || true; echo; }
list_sgid() { find / -perm -02000 -ls 2>/dev/null || true; echo; }
...
print_subsection "/etc/systemd/system"; systemd_custom_units
print_subsection "/etc/profile.d"; profile_d_info
print_section "NETWORKING"
print_subsection "ip"; ip_info
print_subsection "hosts"; hosts_info
print_subsection "resolv"; resolv_info
print_subsection "sockets"; netstat_info
print_section "FILESYSTEM & STORAGE"
print_subsection "findmnt"; mount_info
print_subsection "NFS exports"; exports_info
...
print_subsection "cat /etc/crontab"; crontab_info
print_subsection "systemctl (print only suspicious)"; systemd_services_info
print_section "PERMISSION SURFACES"
print_subsection "Find for files owned by root and group = each group of the invoking user"; files_owned_root
print_subsection "find files/dirs with SUID bit set (perm 04000)"; list_suid
print_subsection "find files/dirs with SGID bit set (perm 02000)"; list_sgid
